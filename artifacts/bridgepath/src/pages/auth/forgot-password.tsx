...
export default function ForgotPassword() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    setSent(true);
    toast({ title: "Check your email", description: "We sent a reset link if that address is registered." });
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ backgroundColor: "#f8f9fc" }}>
      <motion.div className="w-full max-w-md bg-white rounded-2xl shadow-sm border border-gray-100 p-8" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <h1 className="text-2xl font-bold mb-1" style={{ color: DARK }}>Reset password</h1>
        <p className="text-gray-500 text-sm mb-8">We will email you a link to choose a new password.</p>

        {sent ? (
          <p className="text-sm text-gray-600 leading-relaxed">
            If an account exists for <strong>{email}</strong>, you will receive an email shortly. You can close this tab or{" "}
            <Link href="/auth/login" className="font-semibold underline" style={{ color: GREEN }}>return to sign in</Link>.
          </p>
        ) : (
          <form onSubmit={onSubmit} className="space-y-4">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1.5">Email</label>
              <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="h-11" placeholder="you@company.com" autoComplete="email" />
            </div>
            <button
              type="submit"
              disabled={loading}
